...
impl ToRdf for Resource {
  fn to_rdf(&self, graph: &mut Graph) {
    ...
    self.add_link(graph, &s_has_related_object, &p_resource_id, &("urn:uuid:".to_owned() + &self.id), None, None, false);
    if let Some(ref url) = self.url {
      self.add_link(graph, &s_has_related_object, &p_locator, &url, None, None, false);
    }
    self.add_link(graph, &s_has_related_object, &p_has_creator, &self.created_via, None, None, false);
    ...
    self.add_link(graph, &s_has_related_object, &p_has_format, &format!("urn:mimetype:{}", self.format.mime_type), None, None, false);
    ...
    if self.filename.is_some() {
      if self.path.is_some() {
        self.add_link(graph, &s_has_related_object, &p_filename, &format!("{}{}", self.path.clone().unwrap(), self.filename.clone().unwrap()), None, None, false);
      } else {
        self.add_link(graph, &s_has_related_object, &p_filename, &self.filename.clone().unwrap(), None, None, false);
      }
    }
    self.add_link(graph, &s_has_related_object, &p_storage_id, &self.storage, None, None, false);
    ...
    self.add_link(graph, &s_is_issued_by, &p_organisation_name, &self.created_via, None, None, false);
